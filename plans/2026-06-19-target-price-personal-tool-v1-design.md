...
- 台股 current price 與 Bear / Base / Bull 目標價在 API 對外輸出、前端卡片、Markdown 報告及辯論室引用時均四捨五入為整數。
- `impliedUpsidePct` 統一保留 1 位小數；PE ratio 統一保留 1 至 2 位小數，不在不同呈現面使用互相矛盾的精度。
- 格式化只影響呈現與 API 最終值，不用已四捨五入的數字反向參與估值運算。
- `targetPrice` 不加入既有 Agent 分數或最終評級公式；評級仍綜合技術面、基本面、籌碼、風險與資料品質。
- `targetPrice = INSUFFICIENT_DATA` 時，既有評級流程繼續執行，不得 crash、不得因缺少目標價自動升降評級，也不得硬補數字。
- 2330 只按真實回傳的 EPS 與 PE metadata 驗證；EPS 口徑不足或外部 PE 一致性不通過時，接受 `INSUFFICIENT_DATA`，並在 `limitations` 說明可追溯原因。
