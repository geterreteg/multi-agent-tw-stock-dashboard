...
- Render production 已於 PR #3 合併後部署 daily full-market PE cache。穩定後 `/api/analyze` 對 2330 / 2317 / 8299 分別在 16.27 / 16.40 / 15.49 秒回 HTTP 200；當次 Render 到 TWSE 無法取得樣本，兩檔 TWSE 明確降級 `missing`，8299 明確標示 TPEx 第一版不支援，皆未超過前端 30 秒 timeout。
...
1. 持續觀察 Render 到 TWSE 的網路可用性；daily full-market cache 架構已部署，但 2026-06-20 遠端驗收時 TWSE live 全部失敗，因此依契約降級為 `missing`。正式前端已確認可顯示 6 秒 timeout / 樣本不足與 TPEx 不支援原因。
